...
def main():
    ...
    Logs.atuta_logger(f"Auto overtime clock-out job started at {start_time}")
    print(f"Auto overtime clock-out job started at {start_time}")

    try:
        AttendanceService.auto_clock_out_overtime_users()

        end_time = get_current_utc3_time()
        Logs.atuta_logger(f"Auto overtime clock-out job finished at {end_time}")
        print(f"Auto overtime clock-out job finished at {end_time}")

    except Exception as e:
        Logs.atuta_technical_logger(
            "auto_clock_out_overtime_cron_failed",
            exc_info=e
        )
        print("Auto overtime clock-out job failed")
