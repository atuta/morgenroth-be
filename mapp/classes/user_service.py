...
class UserService:
    ...
    def get_latest_organization():
        ...
        try:
            ...
            # In an upsert setup, this will be your primary organization.
            org = OrganizationDetail.objects.first()

            if not org:
                return {"status": "error", "message": "No organization details found"}

            # Prepare data dictionary
            data = {
                "org_uuid": str(org.org_uuid),
                "name": org.name,
            ...
            Logs.atuta_technical_logger(f"ERROR fetching organization: {str(e)}")
    ...
    def create_organization_record(data, logo_file=None):
        ...
        try:
            ...
            if org:
                # --- UPDATE EXISTING ---
                action = "updated"
                org.name = name  # Update to the casing provided in current request
                org.physical_address = data.get("physical_address")
            ...
            return {
                "status": "success",
                "message": f"Organization details {action} successfully",
                "org_uuid": str(org.org_uuid)
            }

        except Exception as e:
            Logs.atuta_technical_logger(f"ERROR processing organization record: {str(e)}")
            return {
                "status": "error",
    ...
    def admin_dashboard_metrics(cls, month=None, year=None):
        ...
        try:
            ...
            for user in users:
                # Sum hours worked in the month using PayrollService helper
                hours_data = PayrollService.get_total_hours_for_period(user, month, year)
                attendance_hours = Decimal(hours_data['message']['total_hours']) if hours_data.get('status') == 'success' else Decimal("0.00")
